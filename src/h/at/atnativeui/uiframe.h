...
static constexpr UINT ATWM_PRETRANSLATE			= (WM_APP + 200);

// Sent to child/owned windows when fonts are updated at container
// level.
static constexpr UINT ATWM_FONTSUPDATED			= (WM_APP + 201);

// Sent to a window to request its ideal size; LPARAM = &SIZE.
// Returns true if handled, false if not.
static constexpr UINT ATWM_GETAUTOSIZE			= (WM_APP + 202);

// Sent to a dockable pane to indicate that its full-screen state
// has changed. WPARAM = (bool)state.
static constexpr UINT ATWM_SETFULLSCREEN		= (WM_APP + 203);

// Internal message used by dockable frame system to handle move
// tracking.
static constexpr UINT ATWM_ENDTRACKING			= (WM_APP + 204);

// Tunneled from ancestors to focus. Return is true if handled,
// false if not.
static constexpr UINT ATWM_PRESYSKEYDOWN		= (WM_APP + 205);
static constexpr UINT ATWM_PRESYSKEYUP			= (WM_APP + 206);
static constexpr UINT ATWM_PREKEYDOWN			= (WM_APP + 207);
static constexpr UINT ATWM_PREKEYUP				= (WM_APP + 208);
...
static constexpr UINT ATWM_QUERYSYSCHAR			= (WM_APP + 209);

// Invalidate layout of parent window.
static constexpr UINT ATWM_INVLAYOUT			= (WM_APP + 210);

// Sent when a modal loop begins to warn that key messages may be
// missed.
static constexpr UINT ATWM_FORCEKEYSUP			= (WM_APP + 211);
...
// procedures.
static constexpr UINT ATWM_CHARUP				= (WM_APP + 212);
...
static constexpr UINT ATWM_INHERIT_DPICHANGED	= (WM_APP + 213);
