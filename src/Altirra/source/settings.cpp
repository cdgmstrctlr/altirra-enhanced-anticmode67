...
void ATSettingsExchangeHardware(bool write, VDRegistryKey& key) {
	...
	if (write) {
		...
		key.setBool("GTIA: EnhancedMode67", gtia.IsAllowingEnhancedMode67());	// CMC
		...
		const bool enhancedMode67 = key.getBool("GTIA: EnhancedMode67", false);		// CMC
		gtia.EnhancedMode67(enhancedMode67);
		g_sim.GetAntic().EnhancedMode67(enhancedMode67);
