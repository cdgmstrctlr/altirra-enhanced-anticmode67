...
void ATGTIARenderer::RenderScanline(int xend, bool pfgraphics, bool pmgraphics, bool mixed) {
	...
	do {
		...
		switch(mPRIOR & 0xc0) {
			case 0x00:
				if (pmgraphics || (mGRACTL & 0x18) != 0 && mMode16XColor) {	// CMC
					if (mbHiresMode)
						RenderMode8(x1, x2);
					else
						RenderLores(x1, x2);
	...
	switch ((mGRACTL & 0x18) + (1 - (uint8)mMode16XColor))	// try to avoid another branch by adding the !bool to the value - sets bit 0 which will cause the code to fall into the default block
	{
		case 0x08:		// add 6 to luma
			...
			for (i = 0; i < w; ++i)
			{
				...
				uint8 pixColor = pixCIdx == 24 ? 0xFF : colorTable[pixCIdx & 0xF];
				if (pixCIdx & 0x20)
				{
		...
		case 0x10:		// add 6 to chroma
			...
			for (i = 0; i < w; ++i)
			{
				...
				if (pixCIdx & 0x20)
					pixColor += 0x60;	// rotate 6 chroma values over. Don't need to AND because the op stays in the upper nybble
				dst[0] = dst[1] = pixColor;
				src++;
				dst += 2;
			}
		...
		case 0x18:		// do both
			...
			for (i = 0; i < w; ++i)
			{
				...
				uint8 pixColor = colorTable[pixCIdx & 0xF];
				if (pixCIdx & 0x20)
					pixColor = ((pixColor & 0xF0) | ((((pixColor << 4) + 0x60) >> 4) & 0xF)) + 0x60;
				dst[0] = dst[1] = pixColor;
				src++;
				dst += 2;
			}
