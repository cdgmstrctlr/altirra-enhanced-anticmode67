...
void ATGTIAEmulator::EnhancedMode67(bool enabled) {
	...
	mpRenderer->ClearMode67ExtraColors();
	if (!enabled)
	{
		mGRACTL &= 0x7;
	}
...
void ATGTIAEmulator::DumpStatus() {
	...
	ATConsolePrintf("GRACTL: %02X%s%s%s\n"
		, mGRACTL
		, mGRACTL & 0x10 ? ", 6-7 chroma shift" : ""
		, mGRACTL & 0x08 ? ", 6-7 luma shift" : ""
...
void ATGTIAEmulator::Render(int x1, int x2) {
	...
	for(int i=0; i<4; ++i) {
		...
		for(SpriteImage *image = sprite.mpImageHead; image; image = image->mpNext) {
			...
			image->mState.Generate(minx2 - image->mX1, (mActivePRIOR & 0x10) ? ((mGRACTL & 0x18) ? 0 : PF3) : (P0 << i), mMergeBuffer + image->mX1); // CMC
			mbPMRendered = true;
		}
	}
}
...
void ATGTIAEmulator::WriteByte(uint8 reg, uint8 value) {
	...
	switch(reg) {
		...
		case 0x1D:
			...
			{
				mGRACTL = value;
				mpRenderer->SetRegisterImmediate(0x1D, value);
			}
			return;
