...
void ATAnticEmulator::EnhancedMode67(bool enable) noexcept	// CMC
{
	mbEnhancedMode67 = enable;
	if (!enable)
	{
		mCHACTL &= 0x7;
		mDblMode67 = 0;
		...
		if (mX == 0) {		// missile DMA
			...
			if (mRowCounter != rowStop) {
				...
				if (mbDLActive) {
					...
					const uint8 mode = kDblModeToMode[(mDLControl & 0x0f) + mDblMode67];	// CMC
					...
					case 15:					// IR mode F: 320x1 graphics, 2 colors/1 lum
						...
						mPFHiresMode = true;
						break;
					case 16:					// IR mode 6: 20x8 characters, 5 colors+	CMC
						mRowCount = 8;
						mPFPushCycleMask = 1;	// 160 pixels normal
						mPFWidthShift = 2;
						mMode16Enabled = true;
						break;
					case 17:					// IR mode 7: 20x16 characters, 5 colors+	CMC
						mRowCount = 16;
						mPFPushCycleMask = 1;	// 160 pixels normal
						mPFWidthShift = 2;
						mMode16Enabled = true;
...
void ATAnticEmulator::AdvanceScanline() {
	...
	if (++mY >= mScanlineLimit) {
		...
		const int mode = kDblModeToMode[(mDLControl & 0x0f) + mDblMode67]; // CMC
...
void ATAnticEmulator::Decode(int offset) {
	...
	static constexpr U16 kExpandMode6[16][16]={
		...
		// CMC duplicate these so both expansion tables can be used w/ the same pointer
		{ 0x0000, 0x0100, 0x1000, 0x1100, 0x0001, 0x0101, 0x1001, 0x1101, 0x0010, 0x0110, 0x1010, 0x1110, 0x0011, 0x0111, 0x1011, 0x1111 },
		{ 0x0000, 0x0200, 0x2000, 0x2200, 0x0002, 0x0202, 0x2002, 0x2202, 0x0020, 0x0220, 0x2020, 0x2220, 0x0022, 0x0222, 0x2022, 0x2222 },
		{ 0x0000, 0x0400, 0x4000, 0x4400, 0x0004, 0x0404, 0x4004, 0x4404, 0x0040, 0x0440, 0x4040, 0x4440, 0x0044, 0x0444, 0x4044, 0x4444 },
		{ 0x0000, 0x0800, 0x8000, 0x8800, 0x0008, 0x0808, 0x8008, 0x8808, 0x0080, 0x0880, 0x8080, 0x8880, 0x0088, 0x0888, 0x8088, 0x8888 },
	...
	{
/*708 */{ 0x0000, 0x0100, 0x1000, 0x1100, 0x0001, 0x0101, 0x1001, 0x1101, 0x0010, 0x0110, 0x1010, 0x1110, 0x0011, 0x0111, 0x1011, 0x1111 },
/*709 */{ 0x0000, 0x0200, 0x2000, 0x2200, 0x0002, 0x0202, 0x2002, 0x2202, 0x0020, 0x0220, 0x2020, 0x2220, 0x0022, 0x0222, 0x2022, 0x2222 },
/*710 */{ 0x0000, 0x0300, 0x3000, 0x3300, 0x0003, 0x0303, 0x3003, 0x3303, 0x0030, 0x0330, 0x3030, 0x3330, 0x0033, 0x0333, 0x3033, 0x3333 },
/*711 */{ 0x0000, 0x0400, 0x4000, 0x4400, 0x0004, 0x0404, 0x4004, 0x4404, 0x0040, 0x0440, 0x4040, 0x4440, 0x0044, 0x0444, 0x4044, 0x4444 },
/*704 */{ 0x0000, 0x0500, 0x5000, 0x5500, 0x0005, 0x0505, 0x5005, 0x5505, 0x0050, 0x0550, 0x5050, 0x5550, 0x0055, 0x0555, 0x5055, 0x5555 },
/*705 */{ 0x0000, 0x0600, 0x6000, 0x6600, 0x0006, 0x0606, 0x6006, 0x6606, 0x0060, 0x0660, 0x6060, 0x6660, 0x0066, 0x0666, 0x6066, 0x6666 },
/*706 */{ 0x0000, 0x0700, 0x7000, 0x7700, 0x0007, 0x0707, 0x7007, 0x7707, 0x0070, 0x0770, 0x7070, 0x7770, 0x0077, 0x0777, 0x7077, 0x7777 },
/*707 */{ 0x0000, 0x0800, 0x8000, 0x8800, 0x0008, 0x0808, 0x8008, 0x8808, 0x0080, 0x0880, 0x8080, 0x8880, 0x0088, 0x0888, 0x8088, 0x8888 },
/*708S*/{ 0x0000, 0x0900, 0x9000, 0x9900, 0x0009, 0x0909, 0x9009, 0x9909, 0x0090, 0x0990, 0x9090, 0x9990, 0x0099, 0x0999, 0x9099, 0x9999 },
/*709S*/{ 0x0000, 0x0A00, 0xA000, 0xAA00, 0x000A, 0x0A0A, 0xA00A, 0xAA0A, 0x00A0, 0x0AA0, 0xA0A0, 0xAAA0, 0x00AA, 0x0AAA, 0xA0AA, 0xAAAA },
/*710S*/{ 0x0000, 0x0B00, 0xB000, 0xBB00, 0x000B, 0x0B0B, 0xB00B, 0xBB0B, 0x00B0, 0x0BB0, 0xB0B0, 0xBBB0, 0x00BB, 0x0BBB, 0xB0BB, 0xBBBB },
/*711S*/{ 0x0000, 0x0C00, 0xC000, 0xCC00, 0x000C, 0x0C0C, 0xC00C, 0xCC0C, 0x00C0, 0x0CC0, 0xC0C0, 0xCCC0, 0x00CC, 0x0CCC, 0xC0CC, 0xCCCC },
/*706S*/{ 0x0000, 0x0D00, 0xD000, 0xDD00, 0x000D, 0x0D0D, 0xD00D, 0xDD0D, 0x00D0, 0x0DD0, 0xD0D0, 0xDDD0, 0x00DD, 0x0DDD, 0xD0DD, 0xDDDD },
/*707S*/{ 0x0000, 0x0E00, 0xE000, 0xEE00, 0x000E, 0x0E0E, 0xE00E, 0xEE0E, 0x00E0, 0x0EE0, 0xE0E0, 0xEEE0, 0x00EE, 0x0EEE, 0xE0EE, 0xEEEE },
/*712S*/{ 0x0000, 0x0F00, 0xF000, 0xFF00, 0x000F, 0x0F0F, 0xF00F, 0xFF0F, 0x00F0, 0x0FF0, 0xF0F0, 0xFFF0, 0x00FF, 0x0FFF, 0xF0FF, 0xFFFF },
/*707 */{ 0x0000, 0x0800, 0x8000, 0x8800, 0x0008, 0x0808, 0x8008, 0x8808, 0x0080, 0x0880, 0x8080, 0x8880, 0x0088, 0x0888, 0x8088, 0x8888 }
///*712S*/{ 0x0000, 0x0F00, 0xF000, 0xFF00, 0x000F, 0x0F0F, 0xF00F, 0xFF0F, 0x00F0, 0x0FF0, 0xF0F0, 0xFFF0, 0x00FF, 0x0FFF, 0xF0FF, 0xFFFF }	// dupe since I only have 15 values I can send over to GTIA (0 is screen background)
	};
	...
	if (!mAbnormalDecodePattern) {
		...
		switch(mode) {
			...
			case 16:	// 20 column text, 9 colors, 8 scanlines CMC
			case 17:	// 20 column text, 9 colors, 16 scanlines CMC
				for (; x < limit; ++x) {
					const int x2 = x - mPFDMAStart;
					const int bit = x2 & 7;
					...
					*dst++ = (uint8)kExpandMode16[mPFDecodeAbCharInv][mAbnormalDecodeShifter >> 4].x;
				}
				break;
...
void ATAnticEmulator::WriteByte(uint8 reg, uint8 value) {
	...
	switch(reg) {
		...
		case 0x01:		// CHACTL
			value &= mbEnhancedMode67 ? 0x1F : 0x07;	// CMC
			...
			if (mCHACTL != value) {
				...
				mDblMode67 = (mCHACTL & 0x08) ? 0x10 : 0x00;	// CMC
				mDblXColor = (mCHACTL & 0x10) != 0;				// CMC
				UpdateCurrentCharRow();
			}
			break;
...
void ATAnticEmulator::EndLoadStateInternal() {
	...
	mDblMode67 = (mbEnhancedMode67 && (mCHACTL & 0x08) != 0) ? 0x10 : 0x00;	// CMC
...
void ATAnticEmulator::UpdateCurrentCharRow() {
	...
	switch (kDblModeToMode[(mDLControl & 0x0f) + mDblMode67]) {	// CMC
	case 2:
	case 3:
	case 4:
...
void ATAnticEmulator::UpdatePlayfieldTiming() {
	...
	const uint8 mode = mDLControl & 15;	// CMC doesn't need change since this is just for the 559 handling
